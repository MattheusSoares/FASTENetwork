function createOrg1() {
  ...
  for i in {0..1}; do
    ...
    set -x
    fabric-ca-client register --caname ca-org1 --id.name peer${i} --id.secret peer${i}pw --id.type peer --tls.certfiles ${PWD}/organizations/fabric-ca/org1/tls-cert.pem
    { set +x; } 2>/dev/null
    ...
    set -x
    fabric-ca-client enroll -u https://peer${i}:peer${i}pw@localhost:7054 --caname ca-org1 -M ${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer${i}.org1.example.com/msp --csr.hosts peer${i}.org1.example.com --tls.certfiles ${PWD}/organizations/fabric-ca/org1/tls-cert.pem
    { set +x; } 2>/dev/null
...
function createOrg2() {
  ...
  for i in {0..3}; do
    ...
    infoln "Registering peer${i}"
    set -x
    fabric-ca-client register --caname ca-org2 --id.name peer${i} --id.secret peer${i}pw --id.type peer --tls.certfiles ${PWD}/organizations/fabric-ca/org2/tls-cert.pem
    { set +x; } 2>/dev/null
    ...
    infoln "Generating the peer${i} msp"
    set -x
    fabric-ca-client enroll -u https://peer${i}:peer${i}pw@localhost:8054 --caname ca-org2 -M ${PWD}/organizations/peerOrganizations/org2.example.com/peers/peer${i}.org2.example.com/msp --csr.hosts peer${i}.org2.example.com --tls.certfiles ${PWD}/organizations/fabric-ca/org2/tls-cert.pem
    { set +x; } 2>/dev/null
